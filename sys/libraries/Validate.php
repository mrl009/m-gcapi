...
class CI_Validate
{
    ...
    protected static $typeMsg = [
        ...
        'chsDashx'    => ':attribute只能是汉字、字母、数字和下划线_及破折号-',
        ...
        'intGt0'      => '验证整型并且大于0',
        'luhn'        => '验证银行卡号的正确性',
        'userPassword1'=> '密码不能有汉字和空格',
        'userPassword2'=> '密码至少包含一个字母',
     ...
     * @return Validate
     */
    public function rule($name,$msg='', $rule = '')
    {
        if (is_array($name)) {
...
//            dump($callback);die;
    ...
    protected function is($value, $rule, $data = [])
    {
        switch ($rule) {
            ...
            case 'phone';
                // 只允许字母和数字 以字母开头
                $result = $this->regex($value, '/^[1][4375896][0-9]{9}$/');
            ...
            case 'chsAlpha':
                ...
                break;
            case 'chs_alpha':
                $result = $this->regex($value, '/^[\x{4e00}-\x{9fa5}a-zA-Z]+[·]?[\x{4e00}-\x{9fa5}a-zA-Z]+$/u');
                break;
            case 'chsAlphaNum':
                // 只允许汉字、字母和数字
            ...
            case 'token':
                ...
                break;
            case 'userPassword1':
                $result = !$this->regex($value, '/.+[\x{4e00}-\x{9fa5}\s]+.+/u
                ');
                break;
            case 'userPassword2':
                $result = $this->regex($value, '/^.*(?![0-9]+$)(?![a-zA-Z]+$)[A-Za-z0-9]+.*$/u');
                break;
            default:
...
//        $no = '7432810010473523';
    ...
    protected function intGt0($value, $reule,$bool=1){
        if (is_array($value)) {
            foreach ($value as $key => $v) {
                if (is_array($v)) {
                    if(!$this->intGt0($v,$reule)){
                        return false;
                    };
                }
     ...
     * 返回 状态值 1,0
     */
    protected function status($value,$reule,$data){
