...
class Juzhifu_model extends Publicpay_model
{
    protected $c_name = 'juzhifu';
    private $p_name = 'JUZHIFU';//商品名称
    //支付接口签名参数 
    ...
    {
        if ($this->code == 7) return $this->buildForm($data);
    ...
    private function getBaseData()
    {
        ...
        $data['funName'] = 'prepay';
        $data['orderTime'] = 30;
        if ($data['payMethod'] == 7) $data['tradeType'] = $this->bank_type;
    ...
    protected function getPayResult($pay_data)
    {
        ...
        $pay_url = $this->url.'/qrcode-front/qrcodeBusiness/common.do';
        $data = post_pay_data($pay_url,$post);
        if (empty($data)) $this->retMsg('接口返回信息错误！');
        //接收参数为JSON格式(非纯json数据) 转化为数组
        $data = json_decode($data,true);
        if (empty($data)) $this->retMsg('接口返回信息格式错误！');
        //判断是否下单成功
        if (empty($data['codeUrl']) || empty($data['retCode']))
        {
