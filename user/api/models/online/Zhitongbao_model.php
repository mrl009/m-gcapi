 ...
 * 智通宝
 *
 * @file        Zhitongbao_model.php
 * @package     user/models/online/
 * @author      Marks
 * @version     v1.0 2017/12/03
 * @created     2017/12/03
 */
class Zhitongbao_model extends MY_Model
{
    ...
    public function call_interface($order_num, $money, $pay_data)
    {
        ...
        if (in_array($pay_data['code'],[7,15,16,19,21,26,33,36])) {
        ...
        if(in_array($pay_data['code'],[2,5,12,13,18])){
            $pay_url = $pay_data['pay_url'][1];
        }
        if(in_array($pay_data['code'],[34,37,28])){
            $pay_url = $pay_data['pay_url'][2];
        }
        // 封装参数
        $postdata=array(
    ...
    public function wy($order_num, $money, $pay_data){
        ...
        //$client_ip = get_ip();
        if(empty($pay_data[2])){
            $pay_url = "https://pay.ztbaopay.com/gateway?input_charset=UTF-8";
    ...
    */
    private function return_code($code, $bank)
    {
        switch ($code) {
            ...
            case 9:
                return 'jdpay_scan';//京东
            case 10:
                return 'bdpay_scan';//百度
            case 12:
                return 'qq_h5api';//QQwap
            case 13:
                return 'jd_h5api';//京东wap
            case 15:
                return 'jd_scan';//JD公众扫码
            case 16:
                return 'tenpay_scan';//QQ公众扫码
            case 17:
                return 'ylpay_scan';//银联
            case 18:
                return 'unionpay_h5api';//银联wap
            case 19:
                return 'yl_scan';//银联公众号
            case 21:
                return 'bd_scan';//百度公众号
            case 26:
                return 'b2c';//收银台
            case 28:
                return 'h5_qq_d';//qqH5
            case 33:
                return 'weixin_scan';//微信公众扫码
            case 34:
                return 'h5_wx_d';//微信H5
            case 36:
                return 'alipay_scan';//支付宝公众扫码
            case 37:
                return 'h5_ali_d';//支付宝h5
            default:
                return 'weixin_scan';//微信
        }
    }
