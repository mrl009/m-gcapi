...
class Tongsao_model extends MY_Model
{
    public $key;
    public $merId;
    public $orderNum;
    ...
    public function call_interface($order_num, $money, $pay_data)
    {
        ...
        if(in_array($pay_data['code'], [1,4,8,9,10])){
            //扫码
            $this->url='http://i.jsxiebo.com/18/passivePay';
        }elseif(in_array($pay_data['code'], [2,5,12,13,20])){
            //。wap跳转
            $this->url='http://i.jsxiebo.com/18/wapPay';
        }
        $this->domain = isset($pay_data['pay_domain']) ? $pay_data['pay_domain'] : '';
        $this->callback = $this->domain . '/index.php/callback/tongsao/callbackurl';//回调地址
        // 组装数据
        $data = $this->fzData($pay_data);
        $return_data=$this->request($data);
        $return_data = iconv('GBK', 'UTF-8',$return_data);
        // var_dump(json_decode($return_data,true));exit;
