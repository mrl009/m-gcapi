 ...
 * 财猫快汇
 *
 * @file        Caimao_model.php
 * @package     user/models/online/
 * @author      Marks
 * @version     v1.0 2017/12/18
 * @created     2017/12/16
 */
class Caimao_model extends MY_Model
{
    ...
    public function call_interface($order_num, $money, $pay_data)
    {
        ...
        if (in_array($pay_data['code'],[2,5,12])) {
            $pay_url =  'https://api.caimao9.com/gateway/api/h5apipay';
        }
        // 封装参数
        $postdata=array(
    ...
    public function wy($order_num, $money, $pay_data){
        ...
        //$private_key=$pay_data['pay_private_key'];  //商户私钥
        //$client_ip = get_ip();
        $pay_url = "https://pay.caimao9.com/gateway?input_charset=UTF-8";
        //$pay_domain = $pay_data['pay_domain'];
        $private_key=$pay_data['pay_private_key'];  //商户私钥
