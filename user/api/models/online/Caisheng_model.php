...
class Caisheng_model extends Publicpay_model
{
    protected $c_name = 'caisheng';
    protected $p_name = 'CS';//商品名称
    private   $k  = '&key=';
    //微信固定金额
    private   $fm  = [20,30,50,100,200,300,500];
    ...
    private function getBaseData()
    {
        ...
        if(in_array($this->code,[1,2])){
            if(!in_array(intval($this->money),$this->fm)){
                return $this->retMsg('请支付20,30,50,100,200,300,500');
            }
        }
    ...
    protected function getPayUrl($pay)
    {
        ...
        if (!empty($pay['pay_url'])) {//
         ...
         //动态acesstoken请求地址
        $tokenurl = $url.'/getAccessToken/merchant';
    ...
    protected function getAccess($pay,$url)
    {
        ...
        if(empty($data['value'])|| $data['success'] <> ture){
    ...
    protected  function getPayResult($paydata){
        ...
        $data = post_pay_data($url,$pay,$head);
        $data = json_decode($data,true);
        if(empty($data['value'])|| empty($data['success'])|| $data['success'] <> true){
            $msg = isset($data['message'])?$data['message']:'接口无数据返回';
            $this -> retMsg("下单失败：{$msg}");
        }
        $payurl = $data['value'];
