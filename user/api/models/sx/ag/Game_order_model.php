    ...
    public function insert_order($data,$all_users)
    {
        ...
        if ($allow_add_report) {
            ...
            if (in_array($user['sn'], WX_DSN)) {
                $this->sx_redis->hincrbyfloat($user['sn'] . ':user:dml', $user['snuid'], $insert_data['valid_betamount']);
            } else {
                $this->redis_select(REDIS_LONG);
                $this->redis_hincrbyfloat('user:dml',  $user['snuid'], $insert_data['valid_betamount']);
            }
