...
class Wechatpay
{
     ...
     * @return array
     */
    public function unifiedOrderH5($params)
    {
        $data = array();
        $data["appid"] = $this->_config["appid"];
        $data["mch_id"] = $this->_config["mch_id"];
        ...
        $data["notify_url"] = $params['notify_url'];
        $data["trade_type"] = $params['trade_type'];
        $data["scene_info"] = $params['scene_info'];//.h5支付必须
        $result = $this->post_h5(self::URL_UNIFIEDORDER, $data);
        return $result;
    }
     ...
     * @return array
     */
    private function post_h5($url, $data, $cert = false)
    {
        $data["sign"] = $this->sign($data);
        $xml = $this->array2xml($data);
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false);
        // curl_setopt ($ch, CURLOPT_REFERER, "http://czhffmyj.com");
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $xml);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_URL, $url);
     ...
     * @param $out_refund_no int 退款单号
     * @param $total_fee float 总金额（单位：分）
     * @param $refund_fee float 退款金额（单位：分）
    ...
    /**
     * 申请退款 - 使用微信订单号
     * @param $transaction_id int 商户订单号
     * @param $out_refund_no int 退款单号
     * @param $total_fee float 总金额（单位：分）
     * @param $refund_fee float 退款金额（单位：分）
     * @param $op_user_id string 操作员账号
