...
class Xinyufu extends Publicpay
{
    ...
    protected function verifySign($data,$key,$name)
    {
        ...
        $sign = $data[$this->sf];
        unset($data[$this->sf]);
        unset($data['remark']);//去掉空的remark
        //获取签名字符串
        $string = implode('|',array_values($data));
        $string .= $this->key_string .'|'. $key;
        $v_sign = md5($string);
        //验证签名是否正确
