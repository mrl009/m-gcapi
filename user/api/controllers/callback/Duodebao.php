...
class Duodebao extends GC_Controller
{
    ...
    public function callbackurl()
    {
        ...
        if (empty($payconf)) {
            ...
            $this->Online_model->online_erro('duodebao', $erroStr);
            die($this->echo_error);
        }
    ...
    private function _get_param()
    {
        ...
        $post['orginal_money'] = $this->P("orginal_money");

        if (!empty($post['dinpaySign'])) {
            $post['dinpaySign'] = base64_decode($post['dinpaySign']);
        }
        return $post;
    }
