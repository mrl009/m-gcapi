...
class Pay789 extends Publicpay
{
    //redis错误标识名称
    protected $r_name = '789';
    //商户处理后通知第三方接口响应信息
    protected $success = "success"; //成功响应
    //异步返回必需验证参数
    protected $sf = 'sign'; //签名参数
    protected $of = 'merNo'; //订单号参数
    protected $mf = 'amout'; //订单金额参数(实际支付金额)
    protected $vm = 1;//是否验证金额(部分第三方实际支付金额不一致)
    protected $vt = 'fen';//金额单位
    protected $tf = 'status'; //支付状态参数字段名
    protected $tc = '1'; //支付状态成功的值
    protected $vs = ['merId','sysNo']; //参数签名字段必需参数
    ...
    protected function verifySign($data,$key,$name)
    {
        ...
        $string = data_to_string($sdata) . $key;
        $v_sign = md5($string);
        //验证签名是否正确
