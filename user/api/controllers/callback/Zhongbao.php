...
class Zhongbao extends Publicpay
{
    //redis错误标识名称
    protected $r_name = 'ZHONGBAO';
    //商户处理后通知第三方接口响应信息
    ...
    protected $vs = ['result','systemorderid','completetime']; 
    ...
    protected function verifySign($data,$key,$name)
    {
        $sign = $data[$this->sf];
        //参与签名的字段信息
        $sign_data['orderid'] = $data['orderid'];
        $sign_data['result'] = $data['result'];
        $sign_data['amount'] = $data['amount'];
        $sign_data['systemorderid'] = $data['systemorderid'];
        $sign_data['completetime'] = $data['completetime'];
        //构造签名字符串
        $k = $this->ks . $key;
