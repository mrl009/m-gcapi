...
class Yuebaoyun extends Publicpay
{
    //redis错误标识名称
    protected $r_name = 'YUEBAOYUN';
    //商户处理后通知第三方接口响应信息
    ...
    protected $vs = ['orderNo','pay_type','mch','time'];
    ...
    protected function verifySign($data,$key,$name)
    {
        ...
        unset($data[$this->sf]);
        //获取签名字符串
        $string = $data['order_id'] . $data['orderNo'];
        $string .= $data['money'] . $data['mch']; 
        $string .= $data['pay_type'] . $data['time'] . md5($key);
        $v_sign = md5($string);
        if ($sign <> $v_sign)
        {
