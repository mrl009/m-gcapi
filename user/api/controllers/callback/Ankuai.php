 ...
 *
 * @author      ssm
 * @package     controllers/callback/Ankuai
 * @version     v1.0 2017/08/30
 * @copyright
 * @link
 */
class Ankuai extends Basepay
{
    /**
     * 支付成功返回的参数
     * @var String
    ...
    protected function _is_sign_succeed($data, $payconf)
    {
        ...
        extract($data);
        $sign1 = md5(sprintf('partner=%s&ordernumber=%s&orderstatus=%s&paymoney=%s%s',$partner, $ordernumber, $orderstatus, $paymoney, $tokenKey));
        if ($sign1 == $sign) {
            return true;
        }
        return false;
