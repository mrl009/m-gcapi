...
class Meilian extends Publicpay
{
    //redis错误标识名称
    protected $r_name = 'MEILIAN';
    //商户处理后通知第三方接口响应信息
    ...
    protected $vs = ['respCode','merOrderId','txnAmt','orgMerOrderId']; //参数签名字段必需参数
    ...
    protected function verifySign($data,$key,$name)
    {
        ...
        $sign = $data[$this->sf];
        unset($data[$this->sf]);
        unset($data['resultMsg']);//字段和文档有差别
        $vdata = array(
            'respCode' => $data['respCode'],
            'merOrderId' => $data['merOrderId'],
            'txnAmt' => $data['txnAmt'],
            'orgMerOrderId' => $data['orgMerOrderId'],
        );
        //获取签名字符串
        ksort($vdata);
        $string = data_to_string($vdata) . $this->ks.$key;
        $v_sign = md5($string);
        //验证签名是否正确
