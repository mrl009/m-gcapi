...
class User extends SX_Controller
{
    ...
    {
        $data = $this->user_api->free();
        $this->ajax_return(['codeId' => $data['codeId'], 'token' => $data['token'], 'list' => $data['list']]);
    }
    ...
    {
        $this->sxuser['status'] = isset($this->sxuser['status']) ? $this->sxuser['status'] : 1;
        $this->sxuser['win_limit'] = isset($this->sxuser['win_limit']) ? $this->sxuser['win_limit'] : 0;
        $data = $this->user_api->update($this->sxuser['merge_username'], $this->sxuser['pwd'], $this->sxuser['status'], $this->sxuser['win_limit']);
        if ($data['codeId'] == 0) {
            $this->load->model('sx/dg/user_model');
            $this->user_model->update_info($this->sxuser['merge_username'], $this->sxuser['pwd'], $this->sxuser['win_limit'], $this->sxuser['status']);
        }
    ...
    {
        $data = $this->user_api->onlineReport();
        if ($data['codeId'] == 0) {
