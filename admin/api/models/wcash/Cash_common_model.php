...
class Cash_common_model extends MY_Model
{
   ...
   public function cash_people_increby($adminId, $money)
   {
     ...
     * @return $arr  array
     */
    public function base_incom_online($name = 'bank',$id=null,$str='*')
    {
   ...
   public function get_pay_sign($data,$key_string,$field='sign',$method='D')
    {
        if (!empty($data) && is_array($data) && !empty($key_string))
        {
            ...
            $sign_string =$this -> ToUrlParams($data).$key_string;
            //拼接字符串进行MD5大写加密
     ...
     * @return string|array
     */
   public function html_trim($param='')
    {
